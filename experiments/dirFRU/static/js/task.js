function runExperiment() {
  ...
  // Per-participant condition: assigned once server-side, held constant across all sessions.
  function getConditionSync(pid) {
    var xhr = new XMLHttpRequest();
    xhr.open(
      "GET",
      "/get_condition?prolific_pid=" + encodeURIComponent(pid),
      false,
    );
    xhr.send(null);
    if (xhr.status !== 200) {
      throw new Error("Could not fetch condition: HTTP " + xhr.status);
    }
    var resp = JSON.parse(xhr.responseText);
    if (!resp.condition) {
      throw new Error("No condition returned from server");
    }
    ...
    timeline: [
      {
        ...
        response_ends_trial: false,
        stimulus:
          "<p>Unfortunately, you do not qualify for this study.</p><p>Please return this HIT to Prolific at this time.</p>",
        choices: jsPsych.NO_KEYS,
        trial_duration: 3000,
        on_finish: function () {
          jsPsych.endExperiment("Please return this study to Prolific.");
  ...
  var instructions = {
    ...
    pages: [
      "<p>In this experiment you will be presented \
            ...
            the list by typing them into the recall box. You will recall as many words as you can. \
            Later, we will give you more instructions on how we want you to recall the words. </p>",
      ...
      saveData()
        ...
        .catch(function (error) {
          ...
          document.body.innerHTML =
            ...
            "<ul><li>Your Prolific ID: <b>" +
            prolific_pid +
            "</b></li>" +
            "<li>Error: " +
            error.message +
            "</li></ul>" +
            "<p>Then enter the completion code <b>" +
            COMPLETION_CODE +
            "</b> on Prolific.</p>" +
            "</div>";
          done();
